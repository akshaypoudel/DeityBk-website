...
function ServiceCard({ service, showFeatures = false }) {
  ...
  return (
    ...
    >
      {/* hover glow */} <div className="pointer-events-none absolute -right-16 -top-16 h-40 w-40 rounded-full bg-accent/20 opacity-0 blur-3xl transition-opacity duration-500 group-hover:opacity-100" />
      ...
      <CardHeading className="mt-6 text-xl">
        {service.title}
      </CardHeading>

      <SmallText className="mt-2.5">
        {service.description}
      </SmallText>
